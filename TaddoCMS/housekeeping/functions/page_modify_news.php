...
?>
	...
	<div class="ImagePreview">
            ...
            <option value=""></option>
            <?php
            $newsimages = opendir('../../Public/Images/news/') or die('Error');  
			while($images = @readdir($newsimages)) {  
				if(!is_dir($images))
				{
					...
					$selected = '';
  					echo '<option value="Public/Images/news/'.$images.'" '.$selected.'>'.$images.'</option>';  
				}
			}
			closedir($newsimages);  
			?>
			...
			<label for="campaignimage">Imagen De Campaña: </label><br />
			<select name="campaignimage" id="campaignimage" style="width:120px">
            <option value=""></option>
            <?php
            $newsimages = opendir('../../Public/Images/campaign/') or die('Error');  
			while($images = @readdir($newsimages)) {  
				if(!is_dir($images))
				{
					...
					$selected = '';
  					echo '<option value="/Public/Images/campaign/'.$images.'" '.$selected.'>'.$images.'</option>';  
				}
			}
			closedir($newsimages);  
			?>
	...
	<button onclick="SubmitForm()" class="PostStory">Publicar</button>